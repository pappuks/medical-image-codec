...
func CompressKlausWithBlocks(in []uint16, width int, height int) []byte {
	...
	for y := 0; y < height; y += maxSize {
		...
		for x := 0; x < width; x += maxSize {
			...
			if singleThreaded {
				...
				results[idx] = compressKlausWithBlocks(in[x:], endX-x, endY-y, width, !tryWithAndWithoutGapRemoval)
				if tryWithAndWithoutGapRemoval {
					withGap := compressKlausWithBlocks(in[x:], endX-x, endY-y, width, true)
				...
				go func(in []uint16, i, width, height, stride int) {
					results[i] = compressKlausWithBlocks(in, width, height, stride, !tryWithAndWithoutGapRemoval)
					if tryWithAndWithoutGapRemoval {
						withGap := compressKlausWithBlocks(in, width, height, stride, true)
...
func compressKlausWithBlocks(in []uint16, width, height, stride int, gap bool) []byte {
	var codes []byte
