...
delta\,+\,Zstandard baseline. Across grayscale images, MIC achieves lossless
...
enough for client-side deployment. These results suggest that 16-bit-native
...
compression: direct coding of \textbf{predicted 16-bit residual streams}. The
...
Impl.\ complexity & 5k & 20k & N/A & N/A & 1.1k \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig5_predictor_ablation.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig6_tablelog_ablation.png}
...
Full 16-bit range (CT) & Delta+RLE+FSE \\
...
dependency chain:
\begin{equation}
  s_{n+1} = \texttt{decTable}[s_n].\textit{newState}
  + \texttt{getBits}\!\left(\texttt{decTable}[s_n].\textit{nbBits}\right).
\end{equation}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig3_multistate_speedup.png}
...
Table~\ref{tab:fse-amd64} shows the Intel Xeon (AMD64) subset for reference.

\begin{table}[htbp]
\centering
\caption{Isolated FSE decompression throughput (MB/s), Intel Xeon @ 2.80\,GHz
(AMD64).}
\label{tab:fse-amd64}
\begin{tabular}{lrrrr}
\toprule
...
(Section~\ref{sec:wavelet-limits}).
...
\subsection{Multi-Frame and RGB Results}

\textbf{Multi-frame (MIC2)}---69-frame breast tomosynthesis ($2457 \times 1890
\times 69$, 614\,MB raw):
...
Independent mode outperforms temporal mode because tomosynthesis frames differ
in X-ray projection angle, not temporal motion.

\textbf{Single-Frame RGB (MICR)}---YCoCg-R + Delta+RLE+FSE applied to the full
image:
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\columnwidth]{figures/fig2_histogram.png}
...
\subsection{Signal-Processing Analysis of Wavelet Limitations}
\label{sec:wavelet-limits}

The CT wavelet regression (1.67$\times$ vs.\ 2.24$\times$ for Delta) is
explained by three structural mechanisms:

\begin{enumerate}
  \item \textbf{Coefficient overflow.} The 5/3 predict step can produce detail
  coefficients exceeding the input dynamic range. For 16-bit inputs, worst-case
  expansion per level is 3/2; for $L=5$ levels, $(3/2)^5 \approx 7.6\times$.

  \item \textbf{The update step reintroduces correlation.} The lifting update
  adjusts low-pass coefficients to preserve the signal mean. For images already
  well-predicted by a simple linear filter, it adds residual energy to the
  low-pass band without a compensating reduction elsewhere.
...
Table~\ref{tab:pareto} summarizes the ratio-throughput-complexity tradeoff.

\begin{table}[htbp]
\centering
\caption{Pareto frontier: ratio vs.\ decompression throughput (approx.\
geo.\ means), with implementation complexity and browser deployability.}
\label{tab:pareto}
\scriptsize
...
Wavelet V2 SIMD & 3.28$\times$ & 780 & 2.0k & Possible \\
HTJ2K (OpenJPH) & 3.15$\times$ & 460 & 20k & No \\
JPEG-LS (CharLS) & 3.44$\times$ & 130 & 5.0k & No \\
$\Delta$+zstd-19 & 3.03$\times$ & 300 & N/A & Partial \\
\bottomrule
\end{tabular}
\end{table}
...
JPEG-LS trades 75\% of decompression speed for 10\% more compression.
MIC-4state-C achieves 4$\times$ the throughput of JPEG-LS at 91\% of its ratio.
HTJ2K's ${\sim}$20,000-line implementation is ${\sim}$18$\times$ larger than
MIC's core pipeline and currently has no production-ready browser decoder.
...
\begin{enumerate}
  \item \textbf{Dataset size.} The dataset (21 images, 10 modalities, 3 public
  repositories) is multi-modality but modest for strong generalization claims.

  \item \textbf{Incomplete baselines.} Byte-shuffle and bitshuffle baselines
  are not yet included.

  \item \textbf{Browser evaluation.} JavaScript performance was measured in
  Node.js, not in browser environments.

  \item \textbf{Predictor ablation.} The comparison covers left-only, average,
  Paeth, and MED. More sophisticated context-based predictors (CALIC, MANIAC)
  were not evaluated.

  \item \textbf{Temporal prediction.} Inter-frame compression underperforms on
  the one available tomosynthesis dataset and needs evaluation on cardiac cine
  MRI and fluoroscopy.

  \item \textbf{Confidence intervals.} Run-to-run variance is $<$2\% on ARM64
  and $<$5\% on AMD64, but formal confidence intervals are not yet reported.

  \item \textbf{No lossy compression}, progressive decoding, or
  region-of-interest coding.

  \item \textbf{No PACS integration or clinical workflow study.}
\end{enumerate}
