...
transform~\cite{legall1988,calderbank1998} via the lifting scheme~\cite{sweldens1996,daubechies1998}
with embedded block coding (EBCOT). It achieves excellent compression
...
(HTJ2K)~\cite{htj2k2019,taubman2022} replaces EBCOT with a faster block coder,
substantially improving decode speed while maintaining JPEG~2000 compatibility.

JPEG-LS~\cite{loco1,jpegls2} uses context-adaptive prediction (MED predictor) followed
...
Context-based adaptive lossless image coding (CALIC)~\cite{wu1997} and
FLIF~\cite{sneyers2016} represent higher-complexity predictive and entropy
coding approaches that achieve strong ratios on natural images but are not
standardized for DICOM and have no practical browser decoder.
...
at 4,096. Modern general-purpose formats such as JPEG~XL~\cite{jpegxl} also
operate on byte-oriented symbol streams. The predominant implementations are
optimized for byte-oriented data.
...
path is branch-free. Fig.~\ref{fig:predictor-ablation} shows the per-image
ratio difference relative to the average predictor across all 21 images.
...
($<$5\% variation). Fig.~\ref{fig:tablelog-ablation} plots per-image compression
ratios for all four settings.
...
images (ARM64, pure Go). Dashed lines show geomeans: 4-state = 1.68$\times$,
2-state = 1.37$\times$.}
...
Fig.~\ref{fig:multistate-speedup} shows the per-image speedup distribution.
...
JavaScript module. Fig.~\ref{fig:pareto} plots this ratio vs.\ decompression
throughput tradeoff for all codec variants (ARM64).
