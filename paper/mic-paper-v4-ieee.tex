...
interleaved entropy decoder provides geomean $+$68\% isolated FSE speedup on
ARM64 ($+$43\% on AMD64) over a single-state implementation without affecting
compressed size. On the reported
...
DICOM~\cite{dicom} supports several established lossless transfer syntaxes, including
...
complexity~\cite{liu2017,clunie2000}. JPEG~2000 and HTJ2K provide strong compression performance but rely
...
\begin{enumerate}
  ...
  instruction-level parallelism, achieving geomean $+$68\% isolated FSE
  speedup on ARM64 ($+$43\% on AMD64) with no compression ratio penalty.
...
transform~\cite{legall1988,calderbank1998} with embedded block coding (EBCOT),
achieving excellent ratios at significant decompression overhead. HTJ2K~\cite{htj2k2019,taubman2022}
replaces EBCOT with a faster block coder, substantially improving decode
speed. JPEG-LS~\cite{loco1,jpegls2} uses context-adaptive MED prediction with
Golomb-Rice coding; in our evaluation it achieves the highest lossless ratio on
all 21 tested images. RLE Lossless (DICOM TS 1.2.840.10008.1.2.5) applies
byte-level PackBits without decorrelation, yielding poor ratios
(typically $<$1.5$\times$). Higher-complexity approaches such as
CALIC~\cite{wu1997} and FLIF~\cite{sneyers2016} achieve strong ratios on
...
when a residual is small (say, $\pm 30$), $X_H$ is nearly deterministic given
$X_L$---yet must still be encoded. On our test images, $I(X_H;\,X_L)$ ranges
from 0.3\,bits/symbol (MR) to 1.1\,bits/symbol (MG3), corresponding to 8--22\%
of total entropy and closely matching MIC's 5--22\% empirical advantage over
Delta\,+\,Zstandard.
...
literals. For generic 16-bit residuals the 16-bit-native representation is
structurally tighter by a factor of $\sim$2 on repeated values.
...
An optional canonical Huffman backend~\cite{schwartz1964} was implemented for comparison. It
...
\textbf{Correctness and signaling.} Correctness follows from partitioning the
output sequence into independent subsequences during encoding and reversing
that assignment during decoding; each chain operates on a strict subset of
positions with no cross-chain data dependency. The bitstream signals the mode
with a 2-byte magic header (\texttt{0xFF,0x02} for two-state;
\texttt{0xFF,0x04} for four-state) followed by a 4-byte symbol count; the
magic byte \texttt{0xFF} cannot appear as a valid single-state FSE header
($\textit{tableLog}=20 > \textit{tableLogAbsoluteMax}=17$), so all formats
coexist without ambiguity.

\textbf{Platform-specific kernels.} On AMD64, BMI2 \texttt{SHLXQ}/\texttt{SHRXQ}
provide 3-operand variable shifts without CL contention; on ARM64,
\texttt{LSLV}/\texttt{LSRV} serve the same role natively. Pure-Go
implementations cover all other targets.
...
Table~\ref{tab:fse-amd64} presents the same isolated FSE benchmark
(\texttt{BenchmarkFSEDecompress4State}) run on the Intel Core Ultra 9
285K (AMD64) for all 21 images, matching the methodology of
Table~\ref{tab:fse-arm64} above.

\begin{table}[tbp]
\centering
\caption{Isolated FSE decompression throughput (MB/s), Intel Core Ultra 9
285K (AMD64). 1-state and 2-state: pure Go; 4-state: BMI2 assembly
...
pure-Go \texttt{zeroBits} bounds-check overhead, yielding $+$127\%---still
large, but far below the $+$590\% on ARM64 where the 1-state pure-Go
decoder stalls to 140~MB/s on this image.

$^\dagger$\textbf{XA1}: the $-$18\% result is within measurement noise for
this small image on a machine with mixed P/E-core topology.
...
gain. For C-level and SIMD multi-state decoder throughput on this
platform, see Table~\ref{tab:decomp-amd64}.
...
\subsection{Encoding Throughput---Full Tables}
...
complementing the ARM64 subset in Table~\ref{tab:encoding}.
...
for HTJ2K~\cite{nohtj2k} or JPEG-LS~\cite{charls}. By contrast, Zstandard has a
...
\begin{itemize}
  ...
  \item \textbf{Multi-state decoder}: geomean $+$68\% isolated FSE speedup
  (ARM64; $+$43\% AMD64) with no ratio penalty.
...
for High-Throughput JPEG~2000 was identified. The OpenJPH open-source library
does not provide a WASM build or npm package.
