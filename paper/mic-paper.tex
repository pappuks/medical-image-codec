...
Compared to HTJ2K, MIC achieves comparable or better lossless compression ratios across all modalities and decompresses 1.1--1.2$\times$ faster on a single core, scaling to 16\,GB/s at 64 cores.
...
\begin{table*}[t]
\centering
\caption{Lossless compression comparison: MIC (Delta+RLE+FSE) vs.\ HTJ2K
...
\midrule
MR  & 0.13 & 2.35$\times$ & \textbf{2.38}$\times$ & 133 & 22\,$\dagger$ & --- \\
CT  & 0.50 & \textbf{2.24}$\times$ & 1.77$\times$ & 135 & 75\,$\dagger$ & --- \\
CR  & 7.18 & 3.63$\times$ & \textbf{3.77}$\times$ & \textbf{242} & 218 & 1.11$\times$ \\
XR  & 10.1 & \textbf{1.74}$\times$ & 1.67$\times$ & \textbf{245} & 212 & 1.16$\times$ \\
MG1 & 9.35 & \textbf{8.57}$\times$ & 8.25$\times$ & \textbf{428} & 351 & 1.22$\times$ \\
MG2 & 9.35 & \textbf{8.55}$\times$ & 8.24$\times$ & \textbf{419} & 342 & 1.22$\times$ \\
MG3 & 27.3 & \textbf{2.24}$\times$ & 2.22$\times$ & \textbf{252} & 226 & 1.12$\times$ \\
MG4 & 26.0 & 3.47$\times$ & \textbf{3.51}$\times$ & \textbf{353} & 311 & 1.14$\times$ \\
\bottomrule
...
MIC decompresses 1.1--1.2$\times$ faster than HTJ2K on a single thread.
...
medical image modalities, while providing higher decompression throughput, especially
at scale.
...
the leading open-source HTJ2K implementation.
MIC achieves comparable or better lossless compression ratios and decompresses 1.1--1.2$\times$
faster on a single thread (Apple M2 Max).
...
A quantitative comparison against HTJ2K (OpenJPH) shows that MIC achieves comparable or better lossless compression ratios across all modalities and decompresses 1.1--1.2$\times$ faster on a single core (Apple M2 Max), scaling to 16\,GB/s on a 64-core ARM64 server --- substantially higher than the throughput achievable by a single-process HTJ2K implementation.
