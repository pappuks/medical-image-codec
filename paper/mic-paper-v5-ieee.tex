...
We evaluate the proposed codec on 21 de-identified Digital Imaging and
Communications in Medicine (DICOM) images spanning MR,
...
is within 1\% relative to High-Throughput JPEG 2000 (HTJ2K, 3.15$\times$)
while being structurally simpler.
Across grayscale images, MIC achieves lossless
compression ratios ranging from 1.7$\times$ to 8.9$\times$. A four-state
interleaved entropy decoder provides a geometric-mean $+$68\% isolated FSE
speedup on ARM64 over a single-state implementation without affecting
compressed size. On the reported ARM64 and AMD64 platforms, the fastest MIC
variant exceeds HTJ2K decompression speed on the majority of tested images
single-threaded; strip-parallel configurations exceed HTJ2K on all 21 images
on both platforms.
...
enough for client-side web deployment, enabling in-browser decoding directly
from object storage with no server-side transcoding. These results suggest that
16-bit-native entropy coding is a useful design point for lossless medical
image compression when decompression throughput, implementation simplicity, and
...
An optional canonical Huffman backend~\cite{schwartz1964} was implemented for
comparison. It typically produces 1--3\% smaller output than FSE but decodes
more slowly. The slowdown arises because the large symbol alphabet requires a
wider decode table with less-predictable variable-length code lookups, whereas
FSE uses a fixed-width table transition (always $\textit{tableLog}$ bits) that
the CPU branch predictor and prefetcher handle more efficiently. Since this
paper focuses on the ratio-throughput tradeoff, FSE is used as the primary
backend.

\subsection{RGB and Multi-Frame Extensions}

MIC includes two extensions:
\begin{itemize}
  \item \textbf{MIC2} for multi-frame grayscale data (the ``2'' denoting version
  2 of the container format), with two modes: independent
  (per-frame spatial Delta+RLE+FSE) and temporal (inter-frame ZigZag-encoded
  residuals).
  \item \textbf{MICR} for single-frame red-green-blue (RGB) images (the ``R''
  denoting RGB), using a reversible YCoCg-R color transform~\cite{ycocgr}
  (a luma/chroma decomposition that maps RGB integers to integers without loss)
  followed by per-plane Delta+RLE+FSE applied to the full image without tiling.
\end{itemize}

These modes are included to demonstrate extensibility; the main contribution
remains the grayscale 16-bit lossless pipeline. Lossless RGB compression ratios
on the NEMA compsamples range from 1.56$\times$ (VL5, photographic) to
6.24$\times$ (US1, ultrasound).
...
Table~\ref{tab:fse-amd64} presents the same isolated FSE benchmark run on the
Intel Core Ultra 9 285K (AMD64) for all 21 images.

\begin{table}[tbp]
\centering
\caption{Isolated FSE decompression throughput (MB/s), Intel Core Ultra 9
285K (x86-64/AMD64). 1-state and 2-state: pure Go; 4-state: BMI2 assembly
(\texttt{fse4StateDecompKernel}). MB/s over uncompressed RLE symbol stream.}
\label{tab:fse-amd64}
\scriptsize
\begin{tabular}{lrrrr}
\toprule
...
\midrule
MR   &   600 &   999 &   828 & $+$38\% \\
CT   &   941 & 1,421 & 1,631 & $+$73\% \\
CR   & 1,994 & 3,958 & 4,535 & $+$127\%$^*$ \\
XR   & 3,195 & 5,028 & 5,771 & $+$81\% \\
MG1  & 3,379 & 4,357 & 4,952 & $+$47\% \\
MG2  & 2,682 & 3,313 & 4,155 & $+$55\% \\
MG3  & 3,048 & 4,613 & 5,517 & $+$81\% \\
MG4  & 5,476 & 6,686 & 6,437 & $+$18\% \\
CT1  & 1,733 & 1,871 & 2,522 & $+$46\% \\
CT2  & 1,780 & 2,504 & 2,485 & $+$40\% \\
MG-N & 2,722 & 4,577 & 4,981 & $+$83\% \\
MR1  & 2,277 & 2,338 & 2,881 & $+$27\% \\
MR2  & 2,421 & 3,609 & 3,769 & $+$56\% \\
MR3  & 1,941 & 1,958 & 2,369 & $+$22\% \\
MR4  & 2,284 & 2,926 & 2,753 & $+$21\% \\
NM1  & 2,078 & 2,587 & 2,407 & $+$16\% \\
RG1  & 2,506 & 4,248 & 4,336 & $+$73\% \\
RG2  & 3,808 & 3,786 & 4,081 & $+$7\% \\
RG3  & 2,315 & 3,285 & 3,925 & $+$70\% \\
SC1  & 3,044 & 3,760 & 3,734 & $+$23\% \\
XA1  & 2,753 & 2,933 & 2,253 & $-$18\%$^\dagger$ \\
\midrule
\textbf{Geomean} & \textbf{2,309} & \textbf{3,091} & \textbf{3,306} & \textbf{$+$43\%} \\
\bottomrule
\end{tabular}
\end{table}

$^*$\textbf{CR on AMD64}: the 4-state BMI2 assembly kernel sidesteps the
pure-Go \texttt{zeroBits} bounds-check overhead, yielding $+$127\%.

$^\dagger$\textbf{XA1}: the $-$18\% result is an outlier likely caused by
small-image effects and mixed P-core/E-core scheduling on the Intel processor.
The image size (2\,MB) is near the threshold where per-chain startup costs
dominate amortized decoding time.

The AMD64 4-state decoder achieves a geometric-mean speedup of $+$43\%
over the 1-state baseline (2-state: $+$34\%), with per-image gains
ranging from $-$18\% (XA1) to $+$127\% (CR). The smaller geomean
relative to ARM64 ($+$68\%) is primarily explained by the absence of
the severe \texttt{zeroBits} stall on ARM64: the Intel out-of-order
engine handles the bounds-checked code path without a dramatic penalty,
raising the 1-state baseline and compressing the relative multi-state
gain. For C-level and Single Instruction, Multiple Data (SIMD) multi-state
decoder throughput on this platform, see Table~\ref{tab:decomp-amd64}.
...
\textbf{Codec variants} (Table~\ref{tab:variants}):
\begin{itemize}
...
\begin{table}[htbp]
\centering
\caption{Codec variant summary. ``Format change'' indicates whether the
compressed bitstream differs from MIC-Go.}
\label{tab:variants}
\scriptsize
\begin{tabular}{lllll}
\toprule
Variant & Language & FSE decoder & SIMD & Format change \\
\midrule
MIC-Go        & Go    & 1-state Go   & No     & Baseline \\
MIC-4state    & Go    & 4-state Go   & No     & Yes (magic header) \\
MIC-4state-C  & C/CGO & 4-state C    & No     & Same as 4-state \\
MIC-4state-SIMD & C/CGO & 4-state C  & BMI2/scalar & Same as 4-state \\
Wavelet+SIMD  & Go/ASM & 1-state Go  & AVX2/scalar & Different pipeline \\
PICS-C-$N$    & C/CGO & 4-state C    & Optional & Per-strip headers \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:ratio-summary} summarizes geometric means and win counts across
all codec variants.
...
throughput, and decoder source lines. JPEG-LS wins on ratio but is the
slowest to decode; HTJ2K is ratio-competitive but ships as ${\sim}$20k lines
of C++ with no browser decoder. MIC's core Delta+RLE+FSE pipeline sits close
to both on ratio while decoding faster than JPEG-LS and fitting in roughly
1k lines---small enough to ship as a zero-dependency JavaScript module.
...
\toprule
Codec & Ratio & Decomp. & Source lines & Browser \\
\midrule
...
the submission date, we did not identify a production-ready npm-distributed
WebAssembly or JavaScript decoder for HTJ2K~\cite{nohtj2k} or
JPEG-LS~\cite{charls}; this landscape may change. By contrast, Zstandard has a
community JavaScript port~\cite{fzstd}, demonstrating that byte-oriented
general-purpose codecs can reach the browser---but native 16-bit medical image
decoding requires a purpose-built implementation such as MIC.
...
\begin{itemize}
  ...
  1.56$\times$--6.24$\times$ single-frame RGB (see Section~\ref{sec:pipeline}).
...
for High-Throughput JPEG~2000 was identified in published npm packages or
official project releases. The OpenJPH open-source library does not provide a
WASM build or npm package at the time of writing.
