% IEEE-format edition of the MIC paper.
% Tables (no figures) are reproduced from mic-paper-v9-ieee-tmi.tex;
% references are the cited subset of the v9 bibliography.
...
% --------------------------------------------------
\title{A 16-Bit-Native Lossless Compression Pipeline\\
       for Medical Images}
...
must see exactly what the scanner produced, and they are ``wide,'' carrying
10--16 bits per pixel rather than the 8 bits of an ordinary photo. That wide
format is where most compression tools perform poorly, because nearly all of
them were adapted from photographic and general-purpose imaging, where pixels
are 8 bits, rather than designed for medical data. This paper describes MIC
(Medical Image Codec), a lossless codec designed natively for medical imaging
that works directly on 16-bit data instead of splitting each pixel into two
bytes. MIC chains three simple stages, a spatial predictor, a
...
\IEEEPARstart{M}{edical} imaging plays a central role in understanding the human
body, and as the instruments that capture these images improve, both their
resolution and the volume of imaging data they generate continue to rise. A
modern scanner produces a great deal of data: a single digital breast
tomosynthesis view can comprise 60 or more image slices and over 600\,MB, and a
full screening study can exceed 2\,GB uncompressed. Hospitals store and move
very large volumes of this data every day, so effective compression directly
affects storage cost, network load, and how quickly images appear in a viewer.
Because a radiologist must see exactly what the scanner produced, this
compression must be lossless: the reconstructed image has to match the original
bit for bit, with no loss of fidelity.
...
Plain run-length coding is simple but offers only modest compression, since it
works on raw bytes with no prediction step.

Lossless compression for medical images is not new, but a common thread runs
through these standards. Their prediction and entropy stages were originally
developed for photographic and general-purpose continuous-tone imaging, where
pixels are typically 8 bits, and were later applied to the 10--16-bit data that
medical scanners produce~\cite{liu2017,clunie2000}. Even JPEG-LS, the most
widely deployed of them in medical imaging, derives from the LOCO-I design for
general continuous-tone images~\cite{loco1}; its adoption for medical data came
afterward. None of these standards was designed from the outset around the
statistics of high-bit-depth medical images. MIC takes the opposite approach
and is built from the ground up for medical images.
...
ceiling on how much a byte-splitting coder is wasting. This sets an
expectation for how much can be recovered by keeping the two bytes together.
Section~\ref{sec:results} reports that MIC produces files 5--22\% smaller than
Delta + Zstandard, a strong byte-oriented baseline that codes the same residuals
after splitting them into bytes; that measured per-image size advantage falls
within the ceiling above, which is consistent with the shared-information
argument being the main source of the gain.
...
MIC is designed around the characteristics of medical images themselves. Such
images tend to contain large homogeneous regions and comparatively few sharp
edges or abrupt changes in contrast, and their pixels are commonly 10--16 bits
wide. Starting from these properties, we surveyed entropy-coding and
signal-processing techniques to identify a combination that compresses this kind
of data well while still decoding at high throughput. The result is a
...
``16-bit residual as one symbol'' concerns the entropy stage only: JPEG-LS and
HTJ2K handle 16-bit images, but their entropy stages (Golomb--Rice, bit-plane
coding) do not treat the predicted residual as a single 16-bit symbol.
Source-line counts are order-of-magnitude context.}
...
16-bit residual as one symbol & No & No & No & Configurable & Yes \\
Large alphabet (symbol cap) & N/A & N/A & 4,096 & Configurable & 65,535 \\
Adaptive table sizing & No & No & No & No & Yes \\
Multi-state ILP & No & No & No & Yes & Yes \\
Strip-parallel decode & No & Yes & Yes & Yes & Yes \\
...
from public GDCM and TCIA collections. Table~\ref{tab:dataset} lists the images.
...
\textbf{Validation on larger datasets.} The evaluation rests on 39 images across eleven
modalities. This is varied enough to exercise a wide range of pixel statistics,
from smooth mammography to noisy computed tomography, but it is far smaller than
a real clinical archive and is not drawn from a controlled multi-site
collection. Several modalities are represented by only two or three images, so
their per-modality means carry little statistical weight, and the geometric
means can be moved noticeably by a single outlier such as a large-background PET
study. The dataset also says nothing about how MIC behaves on acquisition
conditions absent here, such as different reconstruction kernels, vendors,
bit depths beyond the 10--16-bit range tested, or images with substantial
acquisition noise. The numbers should therefore be read as evidence on this
specific set, not as population-level claims; validation on larger,
multi-hospital datasets with per-modality breakdowns is needed before the ratio
and speed margins can be treated as general.
...
\textbf{Stronger baselines.} The comparison covers HTJ2K, JPEG-LS, and Delta +
Zstandard, but omits baselines that would more sharply isolate where MIC's
advantage comes from. In particular, a byte-oriented pipeline with an explicit
...
The evaluation so far operates on the extracted pixel array. To be usable in a
real clinical workflow, MIC has to be carried inside a DICOM object that
existing systems can route, store, and display, which means defining it as a
DICOM transfer syntax. We sketch the path here.
...
baselines; defining MIC as a DICOM transfer syntax; an ARM64 vector kernel for
the wavelet variant; and benchmarking the JavaScript decoder in real browsers.
