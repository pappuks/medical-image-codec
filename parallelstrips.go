...
// CompressParallelStrips8State is like CompressParallelStrips but compresses each
// strip with the 8-state FSE encoder (with graceful fall-back through 4-state /
// 2-state / 1-state inside CompressSingleFrame8State).
func CompressParallelStrips8State(pixels []uint16, width, height int, maxValue uint16, numStrips int) ([]byte, error) {
	if len(pixels) != width*height {
		return nil, fmt.Errorf("parallelstrips: pixel count %d != width*height %d", len(pixels), width*height)
	}
	if numStrips <= 0 {
